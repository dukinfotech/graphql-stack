import { ApolloClient, InMemoryCache, createHttpLink, from, gql, split } from "@apollo/client";
...
const errorLink = onError(({ graphQLErrors, networkError, operation, forward }) => {
  if (graphQLErrors) {
    for (let { message, locations, path, extensions } of graphQLErrors) {
      ...
      if (path && !path.includes("refreshTokens")) {
        console.error(
...
const refreshTokens = () => {
  const refreshAccessToken = getCookie("refreshAccessToken");
  apolloClient.mutate<RefreshTokensMutation>({
    mutation: REFRESH_TOKENS_MUTATION,
    context: {
      headers: {
        authorization: refreshAccessToken ? `Bearer ${refreshAccessToken}` : "",
      }
    }
  })
  .then(res => {
    if (res.data) {
      ...
      setCookie("accessToken", newJwtTokens.accessToken);
      setCookie("refreshAccessToken", newJwtTokens.refreshAccessToken);
      return newJwtTokens.accessToken;
    }
  })
  .catch((error: any) => {
    for (let { message, locations, path, extensions } of error.graphQLErrors) {
      console.error(
        `[GraphQL error]: Message: ${message}, Location: ${JSON.stringify(locations)}, Path: ${path}`
      )
      switch (extensions.code) {
